    ...
    void RunOnPackageInSerial(const Package* package, bool isDebug, Args&&... args)
    {
        ...
        for (auto func : package->GetGlobalFuncsWithBody()) {
            if (ShouldBeAnalysed(*func)) {
                // Debug iff func is not from std.core && it's a Range Analysis
                std::string analysisName = TAnalysis::GetAnalysisName();
                isDebug = func->packageName != "std.core"
                    && analysisName == "range-analysis";
                
                // ? Especially for function "foo", we print the graph
                if (func->srcCodeIdentifier == "foo") {
    ...
    template <typename... Args> void SetUpGlobalVarState(const Package& package, bool isDebug, Args&&... args)
    {
        ...
        for (auto gv : package.GetGlobalVarsWithInit()) {
            if (auto init = gv->GetInitFunc();
                ...
                // ! Turn off debug if init.packageName == "std.core"
                bool shouldDebug = init->packageName == "std.core" ? false : isDebug;
                resultsMap.emplace(init, RunOnFunc(init, shouldDebug , std::forward<Args>(args)...));
            }
        }
    }
