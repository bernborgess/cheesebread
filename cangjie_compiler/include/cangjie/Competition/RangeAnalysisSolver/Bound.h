...
struct Bound {
   ...
   private:
    constexpr long long toKey() const {
        if (type == Type::MinusInfinity)
            return std::numeric_limits<long long>::min();
        if (type == Type::PlusInfinity)
   ...
   public:
    ...
    constexpr bool isMinusInfinity() const {
        return type == Type::MinusInfinity;
    }
    ...
    constexpr void setConstant(int _value) { value = _value; }
    ...
    /**
     * @brief Overload for printing individual boundaries (e.g., -inf, 42,
     * +inf).
     */
    friend std::ostream& operator<<(std::ostream& os, const Bound& bound) {
        switch (bound.type) {
            case Bound::Type::MinusInfinity:
                os << "-2147483648";
                break;
            case Bound::Type::PlusInfinity:
                os << "2147483647";
                break;
            case Bound::Type::Constant:
                os << bound.value;
                break;
            default:
                break;
