...
class Solver {
   ...
   public:
    ...
    /**
     * @brief Main driver that executes all stages of the range analysis
     * pipeline.
     * * Progresses sequentially through the widening/growth phase, symbolic
     * future resolution, and the monotonic narrowing phase until a fixed point
     * is met.
     */
    void resolveSCC();
     ...
     * * Iteratively evaluates dataflow equations. This step propagates and
     * widens bounds upwards until the values stabilize or are forced to
     * infinity.
     */
    void growthAnalysis();
     ...
     * constraints and attempts to contract over-approximated interval
     * boundaries without violating soundness.
     */
