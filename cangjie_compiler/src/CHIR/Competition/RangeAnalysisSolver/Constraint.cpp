...
bool InitializationConstraint::eval(AbstractState& A) {
    ...
    return old_val !=
           std::get<IV>(A[def]);  // Leverages your custom equality operator!
}
...
Bound IntersectionConstraint::resolveBound(const IntersectionBound& b,
                                           const bool isLower,
                                           const AbstractState&) const {
    if (std::holds_alternative<Bound>(b)) return std::get<Bound>(b);
...
IntersectionConstraint IntersectionConstraint::resolveFutures(
    ...
    return IntersectionConstraint(def, operand, resolve(lower_bound, true),
...
bool IntersectionConstraint::eval(AbstractState& A) {
    ...
    if (src.getKind() == IV::Kind::Set) {
        ...
        for (int v : src.getValues()) {
            bool keep = true;
            if (low.isConstant()) keep &= (v >= low.getConstant());
            if (up.isConstant()) keep &= (v <= up.getConstant());
            if (keep) vals.emplace_back(v);
        }

        if (!vals.empty()) std::get<IV>(A[def]).addConstant(vals);
    }
    ...
    // Source is already an interval.
        ...
        if (low.isConstant()) {
            if (lower.isMinusInfinity())
                ...
                lower.setConstant(
                    std::max(lower.getConstant(), low.getConstant()));
        }
        ...
        if (up.isConstant()) {
            if (upper.isPlusInfinity())
                ...
                upper.setConstant(
                    std::min(upper.getConstant(), up.getConstant()));
        }
...
bool AddConstraint::eval(AbstractState& A) {
    ...
    auto addLower = [](const Bound& a, const Bound& b) -> Bound {
    ...
    auto addUpper = [](const Bound& a, const Bound& b) -> Bound {
    ...
    unsigned s1 =
        (lhs.getKind() == IV::Kind::StridedInterval) ? lhs.getStride() : 1;

    unsigned s2 =
        (rhs.getKind() == IV::Kind::StridedInterval) ? rhs.getStride() : 1;
...
bool DivConstraint::eval(AbstractState& A) {
    ...
    if (lhs.getKind() == IV::Kind::Set && rhs.getKind() == IV::Kind::Set) {
        ...
        for (int l : lhs.getValues())
            for (int r : rhs.getValues())
                if (r != 0)  // division by zero is UB; that combination is
                             // unreachable
                    consts.emplace_back(l / r);
    ...
    if (divisorMayBeZero || !lLower.isConstant() || !lUpper.isConstant() ||
        ...
        std::get<IV>(A[def]).setAsInterval(Bound::minusInfinity(),
    ...
    std::get<IV>(A[def]).setAsInterval(Bound::constant(lo), Bound::constant(hi),
                                       1);
    return old_val != std::get<IV>(A[def]);
}
...
bool ModConstraint::eval(AbstractState& A) {
    ...
    if (lhs.getKind() == IV::Kind::Set && rhs.getKind() == IV::Kind::Set) {
        ...
        for (int l : lhs.getValues())
            for (int r : rhs.getValues())
                if (r != 0) consts.emplace_back(l % r);
    ...
    int m = std::max(std::abs(rLower.getConstant()),
                     std::abs(rUpper.getConstant()));
    if (m == 0) {
        std::get<IV>(A[def])
            .setAsBottom();  // the divisor's range is exactly {0}
        return old_val != std::get<IV>(A[def]);
    }
    std::get<IV>(A[def]).setAsInterval(Bound::constant(-(m - 1)),
                                       Bound::constant(m - 1), 1);
    return old_val != std::get<IV>(A[def]);
}
...
bool ShiftLeftConstraint::eval(AbstractState& A) {
    ...
    if (lLower.isConstant() && lUpper.isConstant() && rLower.isConstant() &&
        ...
        int lo = *std::min_element(corners, corners + 4);
        int hi = *std::max_element(corners, corners + 4);
        std::get<IV>(A[def]).setAsInterval(Bound::constant(lo),
                                           Bound::constant(hi), 1);
    } else {
        std::get<IV>(A[def]).setAsInterval(Bound::minusInfinity(),
                                           Bound::plusInfinity(), 1);
...
bool BitwiseAndConstraint::eval(AbstractState& A) {
    ...
    if (lLower.isConstant() && lLower.getConstant() >= 0 &&
        ...
        std::get<IV>(A[def]).setAsInterval(Bound::constant(0),
                                           Bound::constant(hi), 1);
    } else {
        std::get<IV>(A[def]).setAsInterval(Bound::minusInfinity(),
                                           Bound::plusInfinity(), 1);
...
bool BitwiseXorConstraint::eval(AbstractState& A) {
    ...
    if (lLower.isConstant() && lLower.getConstant() >= 0 &&
        rLower.isConstant() && rLower.getConstant() >= 0 &&
        lUpper.isConstant() && rUpper.isConstant()) {
        int hi = upperBoundForOrXor(
            std::max(lUpper.getConstant(), rUpper.getConstant()));
        std::get<IV>(A[def]).setAsInterval(Bound::constant(0),
                                           Bound::constant(hi), 1);
    } else {
        std::get<IV>(A[def]).setAsInterval(Bound::minusInfinity(),
...
bool NegConstraint::eval(AbstractState& A) {
    ...
    Bound newLower = hi.isPlusInfinity() ? Bound::minusInfinity()
                                         : Bound::constant(-hi.getConstant());
    Bound newUpper = lo.isMinusInfinity() ? Bound::plusInfinity()
                                          : Bound::constant(-lo.getConstant());
...
bool IncConstraint::eval(AbstractState& A) {
    ...
    Bound lo = src.getLower(), hi = src.getUpper();
    Bound newLower =
        lo.isMinusInfinity() ? lo : Bound::constant(lo.getConstant() + 1);
    Bound newUpper =
        hi.isPlusInfinity() ? hi : Bound::constant(hi.getConstant() + 1);
    std::get<IV>(A[def]).setAsInterval(newLower, newUpper, src.getStride());
    return old_val != std::get<IV>(A[def]);
}
...
bool BitwiseNotConstraint::eval(AbstractState& A) {
    ...
    Bound lo = src.getLower(), hi = src.getUpper();
    Bound newLower = hi.isPlusInfinity()
                         ? Bound::minusInfinity()
                         : Bound::constant(-(hi.getConstant()) - 1);
    Bound newUpper = lo.isMinusInfinity()
                         ? Bound::plusInfinity()
                         : Bound::constant(-(lo.getConstant()) - 1);
    std::get<IV>(A[def]).setAsInterval(newLower, newUpper, src.getStride());
    return old_val != std::get<IV>(A[def]);
}
    ...
    if (lhs.getKind() == IV::Kind::Set && rhs.getKind() == IV::Kind::Set) {
        ...
        auto multiplyBounds = [](const Bound& x, const Bound& y) {
            ...
            if (l.isMinusInfinity()) {
                ...
                if (h.isConstant()) {  // C * C
                    return Bound::constant(l.getConstant() * h.getConstant());
                } else {
...
// --- InitializationConstraint (v = c) ---
InitializationBoolConstraint::InitializationBoolConstraint(std::string var,
                                                           bool c)
    : Constraint(std::move(var)), constant(c) {}
...
bool InitializationBoolConstraint::eval(AbstractState& A) {
    ...
    return old_val !=
           std::get<BV>(A[def]);  // Leverages your custom equality operator!
}
...
bool EqualConstraint::eval(AbstractState& A) {
    ...
    if (lhs.getKind() == IV::Kind::Set && rhs.getKind() == IV::Kind::Set) {
        ...
        vals.push_back(true);
        if (lhs.getLower() != lhs.getUpper()) vals.push_back(false);
...
bool NotEqualConstraint::eval(AbstractState& A) {
    ...
    if (lhs.getKind() == IV::Kind::Set && rhs.getKind() == IV::Kind::Set) {
        ...
        vals.push_back(false);
        if (lhs.getLower() != lhs.getUpper()) vals.push_back(true);
    } else if (lhs < rhs || lhs > rhs) {
        vals.push_back(true);
    } else {
        vals.push_back(true);
...
bool LessThanConstraint::eval(AbstractState& A) {
    ...
    if (lhs.getKind() == IV::Kind::Set && rhs.getKind() == IV::Kind::Set) {
        ...
        for (int l : lhs.getValues())
            for (int r : rhs.getValues()) (l < r ? anyTrue : anyFalse) = true;
...
bool RangeConstraint::eval(AbstractState& A) {
    ...
    if (low.isConstant() && high.isConstant() &&
        low.getConstant() > high.getConstant()) {
        std::get<IV>(A[def]).setAsBottom();
        return old_val != std::get<IV>(A[def]);
    }
