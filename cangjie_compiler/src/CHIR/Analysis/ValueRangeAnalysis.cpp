...
std::optional<std::unique_ptr<ValueRange>> SIntRange::Join(const ValueRange& rhs) const
{
    ...
    if (!domain.IsSame(rhsRange.domain) || (domain.NumericBound().IsFullSet() && !domain.SymbolicBounds().Empty())) {
...
RangeDomain ValueAnalysis<RangeValueDomain>::globalState{
    &globalChildrenMap, &globalAllocatedRefMap, nullptr, &globalAllocatedObjMap, &globalRefPool, &globalAbsObjPool};
...
void RangeAnalysis::HandleNormalExpressionEffect(RangeDomain& state, const Expression* expression)
{
    ...
    // TODO: Find out HOW this flag is set. (--chir-opt-debug)
    // GOAL: Only show debug info for the user code (not stdlib)
    // ? Deduce if this is user code, not core + lib code.
    if (std::string packageName = this->func->packageName;
        isDebug &&  packageName != "std.core" &&
        !expression->GetResult()->GetType()->IsRef()) {
